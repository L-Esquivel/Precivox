...
def get_mermas():
    ...
    try:
        ...
        current_app.logger.error(f"Error en get_mermas: {e}")
        return jsonify({"error": "Error al obtener los registros de merma"}), 500
    ...
    try:
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            ...
            if id_producto:
                ...
                current_app.logger.debug(f"Merma (Producto): ID {id_producto}, Nombre '{item.get('nombre')}', Precio de Venta: {item.get('precio', 0)}")
            ...
            current_app.logger.debug(f"Merma: Cantidad {cantidad}, Costo Unitario {unit_cost}, Costo Calculado de Pérdida: {loss_cost}")
            ...
            register_log(f"Registró merma de '{description}' con un costo de ${loss_cost}")
            return jsonify({"message": "Registro de merma agregado exitosamente"}), 201
    except Exception as e:
        conn.rollback()
        current_app.logger.error(f"Error en create_merma: {e}")
        return jsonify({"error": "Error al agregar el registro de merma"}), 500
...
def delete_merma(id):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            register_log(f"Eliminó registro de merma ID {id}")
            return jsonify({"message": "Registro de merma eliminado exitosamente"})
    except Exception as e:
        conn.rollback()
        current_app.logger.error(f"Error en delete_merma: {e}")
        return jsonify({"error": "Error al eliminar el registro de merma"}), 500